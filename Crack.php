...
//    }
    ...
    private static function get_chars($js)
    {
        ...
        //     return ZA__;
        // };
        preg_match_all('/var\s+[^=]+=\s*function\(\w+\)\{\s*[\'\"]return\s*\w+\s*[\'\"];\s*return\s+\w+;\s*\};/x', $js, $l);
        foreach ($l[0] as $function_k => $function) {
            if (!empty($function)) {
        ...
        preg_match_all('/var\s+[^=]+=\s*function\(\)\{\s*
        [\'\"]return\s*\w+\s*[\'\"];\s*
                    return\s+[\'\"][^\'\"]+[\'\"];\s*
                    \};/x', $js, $l);
        foreach ($l[0] as $function_k => $function) {
            if (!empty($function)) {
                preg_match('/var\s+([^=]+)=\s*function\(\)\{\s*
                    [\'\"]return\s*\w+\s*[\'\"];\s*
                    return\s+[\'\"]([^\'\"]+)[\'\"];\s*
                    \};/x', $function, $function_name_arr);
        ...
        //                return iU__;
        //            })('9F')
        preg_match_all('/\(function\(\w+\)\s*\{\s*
        [\'\"]return[^\'\"]+[\'\"];\s*
                    return\s*\w+;
                \}\)\([\'\"][^\'\"]*[\'\"]\)/x', $js, $l);
        foreach ($l[0] as $function_k => $function) {
            if (!empty($function)) {
                preg_match('/\(function\(\w+\)\s*\{\s*
                    [\'\"]return[^\'\"]+[\'\"];\s*
                    return\s*\w+;
                \}\)\(([\'\"][^\'\"]*[\'\"])\)/x', $function, $function_name_arr);
                $js = str_replace($function, $function_name_arr[1], $js);
